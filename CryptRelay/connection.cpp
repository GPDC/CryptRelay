...
void TCPConnection::clientCompetitionThread(void* instance)
{
	...
	while (globalWinner != SERVER_WON && globalWinner != CLIENT_WON) 
	{
		...
		if ((iFeedback = self->connectToTarget()) == 0) return; //1;to change timeout options check out setsockopt: http://www.freebsd.org/cgi/man.cgi?query=setsockopt&sektion=2								
			...
			if (globalWinner != SERVER_WON){	//make sure the server hasn't already won during the time it took to attempt a TCPConnection
				std::cout << "CTHREAD :: ";
...
bool TCPConnection::serverGetAddress()
{
	if (global_verbose == true)
...
bool TCPConnection::clientGetAddress()
{
	if (global_verbose == true)
		std::cout << "Retreiving info: IP address and port...\n";
	// Resolve the LOCAL server address and port.
...
bool TCPConnection::bindToListeningSocket()//my local ip address and port
{
	if (global_verbose == true)
...
void TCPConnection::putSocketInFdSet()//STATUS: NOT USED, what was this for?
{
	return;
}

int TCPConnection::connectToTarget()
{
	if (global_verbose == true)
...
bool TCPConnection::listenToListeningSocket()
{
...
void TCPConnection::closeTheListeningSocket()//get rid of this?
{
	if (global_verbose == true)
...
bool TCPConnection::shutdownConnection()
{
